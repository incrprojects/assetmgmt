...
final class Utils {
    ...
    private static function throwEncodeError(int $code, $data): void {
        switch($code) {
            case JSON_ERROR_DEPTH : $msg = 'Maximum stack depth exceeded';
            break;
            case JSON_ERROR_STATE_MISMATCH : $msg = 'Underflow or the modes mismatch';
            break;
            case JSON_ERROR_CTRL_CHAR : $msg = 'Unexpected control character found';
            break;
            case JSON_ERROR_UTF8 : $msg = 'Malformed UTF-8 characters, possibly incorrectly encoded';
            break;
        ...
        if(is_string($data)&& !preg_match('//u', $data)) {
           ...
           , $data);
            if(!is_string($data)) {
                ...
                throw new \RuntimeException('Failed to preg_replace_callback: ' . 
                                            $pcreErrorCode . 
                                            ' / ' . 
